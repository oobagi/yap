...
{
    /// <summary>
    /// OpenAI text formatting provider.
    /// Mirrors callOpenAI() from the macOS TextFormatter.
    /// </summary>
    public class OpenAiFormatter : IFormattingProvider
    {
        ...
        public OpenAiFormatter(string apiKey, string? model = null, string style = "formatted")
        {
            _apiKey = apiKey;
        ...
        public async Task<TranscriptionResult> FormatAsync(string text)
        {
            ...
            Logger.Log($"Formatting with OpenAI, model={_model}, style={_style}");
            ...
            {
                Logger.Log($"OpenAI format error: {ex.Message}");
